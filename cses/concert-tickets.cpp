...
int main() {
  ...
  multiset<int> h;
//   h.reserve(n);
  REP(i, n) { int t; cin >> t; h.insert(t); }
//   sort(h.begin(), h.end());
  ...
  REP(i, m) {
      ...
      if (h.empty()) { println(-1); continue; }
      
      auto first_greater = h.upper_bound(t);

      if (first_greater == h.begin()) {
          // no such a cheap ticket
          println(-1);
      } else {
          auto last_leq = prev(first_greater);
          println(*last_leq);
          h.erase(last_leq);
      }
  }
